...
import cs301.amazebyjosephpalazzo.gui.Constants;
import cs301.amazebyjosephpalazzo.gui.Controller;
...
public class MapDrawer {
	 ...
	 * @param c
	 */
	public MapDrawer(int width, int height, int map_unit, int step_size, Cells seencells, int map_scale, Controller c){
	...
	public void draw_map(MazePanel panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				panel.setToColorWhiteOrGray(seencells, x, y, CardinalDirection.North);
				...
				if ((seencells.hasWall(x,y, CardinalDirection.North) || showMaze) && theCondition)
					...
					panel.MazeDrawLine(nx1, ny1, nx2, ny1);
				...
				panel.setToColorWhiteOrGray(seencells, x, y, CardinalDirection.West);
				if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
					//gc.drawLine(nx1, ny1, nx1, ny2);
					panel.MazeDrawLine(nx1, ny1, nx1, ny2);
			}
	...
	public void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
		...
		panel.MazeFillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz);
		...
		panel.MazeDrawLine(ctrx, ctry, aptx, apty);
		//gc.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		panel.MazeDrawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		//gc.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
		panel.MazeDrawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	...
	public void draw_solution(MazePanel panel, int offx, int offy, int px, int py) {
		...
		while (distance > 1) {
			...
			panel.MazeDrawLine(nx1, ny1, nx2, ny2);
