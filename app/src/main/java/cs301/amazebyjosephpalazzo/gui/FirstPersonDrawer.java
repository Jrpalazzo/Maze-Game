...
import cs301.amazebyjosephpalazzo.gui.MazePanel;
import cs301.amazebyjosephpalazzo.generation.BSPBranch;
import cs301.amazebyjosephpalazzo.generation.BSPLeaf;
import cs301.amazebyjosephpalazzo.generation.BSPNode;
import cs301.amazebyjosephpalazzo.generation.Cells;
import cs301.amazebyjosephpalazzo.generation.Seg;
 ...
 */
/*
 * Used the functions from mazePanel to draw, rather than using the AWT graphics
 */
public class FirstPersonDrawer {
	 ...
	 * @param wrapper for drawing on the buffer image
	 ...
	 */
	public void draw(MazePanel wrapper, int x, int y, int walkStep, int ang) {
	...
	*/
	private void drawBackground(MazePanel wrapper) {
		wrapper.setColor(Color.BLACK);
		wrapper.fillRect(0, 0, viewWidth, viewHeight/2);
		wrapper.setColor(Color.GRAY);
		wrapper.fillRect(0, viewHeight/2, viewWidth, viewHeight/2);
	}
	/**
	...
	private void drawSegment(Seg seg) {
		...
		wrapper.setColor(seg.getRed(), seg.getGreen(), seg.getBlue());
